...
async function route(r:Request){
 ...
 if(path[0]==="resolve"&&method==="GET"){
  ...
  if(track.provider==="youtube"){
   ...
   if(track.kind==="playlist"&&env.YOUTUBE_API_KEY){const data=await google("playlistItems",{part:"snippet",playlistId:track.id,maxResults:"50"});return json({track,items:(data.items||[]).filter((x:any)=>x.snippet.resourceId.videoId).map((x:any)=>({provider:"youtube",kind:"track",id:x.snippet.resourceId.videoId,title:x.snippet.title,artist:x.snippet.videoOwnerChannelTitle||"YouTube",url:`https://www.youtube.com/watch?v=${x.snippet.resourceId.videoId}`})),more:!!data.nextPageToken});}
  }return json({track});
 }
