...
export function usePlayer(selection:Selection,onEnded:()=>void,onState?:(playing:boolean,position:number)=>void){
 const mount=useRef<HTMLDivElement>(null),yt=useRef<any>(null),sp=useRef<any>(null),device=useRef(""),ready=useRef(false),pendingPlay=useRef(false),current=useRef(selection),ended=useRef(onEnded),stateCb=useRef(onState),spotifyStarted=useRef(false);
 ...
 async function toggle(){if(current.current.track.provider!=="youtube")return;setError("");try{if(!ready.current){pendingPlay.current=true;setStatus("Starting player…");return;}yt.current.getPlayerState()===1?yt.current.pauseVideo():yt.current.playVideo();}catch(e){report(e);}}
